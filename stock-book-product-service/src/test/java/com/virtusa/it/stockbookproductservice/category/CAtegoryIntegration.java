...
public class CAtegoryIntegration {
	...
	public void getCategoryById() throws Exception
	{
		...
		String getResponse = mockMvc.perform(get("/api/category/"+theCategory.getId()))
	...
	{
		String action = mockMvc.perform(get("/api/category/55884"))
	...
	String result = 
			mockMvc.perform(put("/api/category/"+category5.getId())
	  ...
	  @Test //negative test
	  public void updateProductByWrongId() throws  Exception
	  {
		  
		  Category updateCategory = new Category(45L,"updated category5");
		  
		  String result = 
					mockMvc.perform(put("/api/category/45")
							.contentType(MediaType.APPLICATION_JSON)
							.content(new Gson().toJson(updateCategory)))
					.andExpect(status().isBadRequest())
				  .andReturn().getResponse().getContentAsString();
